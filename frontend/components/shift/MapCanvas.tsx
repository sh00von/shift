...
export default function MapCanvas({ basemap }: { basemap: string }) {
  ...
  return (
    ...
    >
      ...
      {/* Shorelines */}
      {visibility.shorelines && shorelines && (
        <GeoJSON
          ...
          style={(f: any) => ({
            ...
            weight: 2.5,
            opacity: opacity.shorelines,
          })}
          onEachFeature={(f, layer) => {
            if (f.properties?.date_str) {
              layer.bindTooltip(
                `<div class="font-sans text-xs"><b>Survey Epoch:</b> ${f.properties.date_str}</div>`,
                { sticky: true }
              );
            }
          }}
      ...
      {visibility.baseline && baseline && (
        <GeoJSON
          ...
          onEachFeature={(_f, layer) => {
            layer.bindTooltip(
              `<div class="font-sans text-xs"><b>Baseline Reference Line</b></div>`,
              { sticky: true }
            );
          }}
        />
      ...
      {/* Cast Transects */}
      {visibility.transects && transects && (
        <GeoJSON
          ...
          onEachFeature={(f, layer) => {
            ...
            if (tid !== undefined) {
              layer.bindTooltip(
                `<div class="font-sans text-xs"><b>Transect #${tid}</b></div>`,
                { sticky: true }
              );
      ...
      {/* Rate Choropleth Transects */}
      {visibility.rates && choropleth?.geojson?.features?.length ? (
        <GeoJSON
          ...
          style={(f: any) => {
            ...
            return {
              color: f.properties?.color || "#16a34a",
              weight: isSelected ? 5.5 : 3.5,
              opacity: isSelected ? 1 : opacity.rates,
            };
          }}
          onEachFeature={(f, layer) => {
